...
Window::Window(Widget *parent, const std::string &title, bool ism)
    : Widget(parent), mTitle(title), mButtonPanel(nullptr), mModal(false), mDrag(false), ismovable(ism), osize(Eigen::Vector2i(0, 0))  { }
...
bool Window::mouseDragEvent(const Vector2i &, const Vector2i &rel,
    ...
    if ((ismovable)&&(mDrag && (button & (1 << GLFW_MOUSE_BUTTON_1)) != 0)) {
        mPos += rel;
