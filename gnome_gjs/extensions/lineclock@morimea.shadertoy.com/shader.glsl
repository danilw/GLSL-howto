...
float drawhours(vec2 uv){
    ...
    uv/=3.1;
    uv.x=-uv.x;
    return showNum(vec2(-mod((uv.x)/(.01),2./3.1)/(0.15),uv.y*80.),t);
}
...
{
    vec2 uv = ((fragCoord-vec2(custom_posx_shift,custom_posy_shift)-.5*iResolution.xy) / iResolution.y)*vec2(9./16.,1.);//vec2 uv = fragCoord/iResolution.xy-0.5;
    uv*=.75;
    ...
    fragColor = vec4(colorMinutes*vline1_aa*color_lines,1.);
    fragColor += vec4(tri*colorTri*color_lines,1.);
    fragColor += vec4(dmins*colorTextM*color_lines,1.);
    fragColor+=vec4(dhours*colorTextH*color_lines,1.);
    fragColor.w=max(max(vline1_aa,dmins),max(dhours,tri))*gradient;
    fragColor.w=clamp(fragColor.w*alpha_val*1.25,0.,1.);
    fragColor=clamp(fragColor,0.,1.);
    fragColor.rgb=sqrt(fragColor).rgb*1.25;
    
    //fragColor.rgb=mix(vec3(1.),texture(iChannel0,fragCoord/iResolution.xy).rgb,fragColor.w);
    
    fragColor.rgb*=gradient*alpha_val;
}
